...
def nms_v8_strict(boxes, scores, staff_space):
    ...
    while len(idxs) > 0:
        ...
        # ★改良：和音を消さないよう、重複判定の距離を 0.7 -> 0.45 に縮小
        duplicate = (dist < staff_space * 0.45)
...
def detect_note_heads_v8(gray_img, staff_space, threshold_val, staves):
    ...
    # ==========================================
    # ★改良1：先に白抜き音符（2分・全音符）の穴を埋める
    # ==========================================
    close_k_size = int(staff_space * 0.8) 
    ...
    # ==========================================
    # ★改良2：穴を埋めた後に、五線や符幹（細い線）を溶かす
    # ==========================================
    open_k_size = max(3, int(staff_space * 0.55))
    ...
    res = cv2.matchTemplate(opened, template, cv2.TM_CCOEFF_NORMED)
    ...
    for (x, y) in zip(*loc[::-1]):
        ...
        # ★改良：和音がくっついて形が崩れても拾えるよう、厳しい輪郭チェックを撤廃
...
st.title("🎼 ドレミ付与ツール V8 (和音＆白抜き音符対応版)")
st.write("2分音符や和音もしっかり検出しつつ、五線や余計な記号を除外します。")
