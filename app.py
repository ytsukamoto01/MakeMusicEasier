...
def detect_note_heads_v8(gray_img, staff_space, user_threshold, staves):
    ...
    for (x, y) in zip(*loc[::-1]):
        ...
        # 2) 矩形のアスペクト比（横長の楕円に近いものだけ）
        aspect = w / float(h)
        if not (1.1 <= aspect <= 2.3):
            continue

        # 3) 面積チェック（小さすぎるゴミ・大きすぎる記号を除外）
        area = w * h
        ...
        if circularity < 0.45:  # 0.4〜0.5 くらいで調整
            continue
...
sens = st.sidebar.slider("検出感度（高いほどたくさん検出）", 0.2, 0.9, 0.45, 0.01)
