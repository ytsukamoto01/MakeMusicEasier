...
def detect_staff_lines_precise(pil_img):
    ...
    if len(peaks) < 2: return [], gray
    staff_space = np.median(np.diff(peaks))
    
    # 【修正点】カーネル長をstaff_spaceの1.5倍に変更（以前は2倍）。より細かい線も捉える
    kernel_len = int(staff_space * 1.5)
...
def non_max_suppression(boxes, overlapThresh):
    ...
    while len(idxs) > 0:
        ...
        yy2 = np.minimum(y3[i], y3[idxs[:last]])
...
def detect_note_heads_precise(gray_img, staff_space, user_threshold):
    ...
    # 【修正点】カーネルサイズをstaff_spaceの0.4倍に調整（以前は0.45倍）
    ...
    # 【修正点】楕円の傾きを-15度に変更（以前は-20度）。より一般的な音符に近づける
    cv2.ellipse(template, center, axes, -15, 0, 360, 255, -1)
...
def analyze_score_v2(pil_img, user_threshold):
    ...
    # 【修正点】五線のグループ化の条件を緩和（間隔の許容範囲を0.5～1.5倍に）
    staves = []
    for i in range(len(staff_y_coords)):
        y1 = staff_y_coords[i]
        # 既存の段と近すぎる場合はスキップ（重複防止）
        if staves and y1 < staves[-1][4] + staff_space * 1.5:
            continue
        ...
        for j in range(i+1, len(staff_y_coords)):
            yj = staff_y_coords[j]
            if staff_space * 0.5 < (yj - last_y) < staff_space * 1.5:
    ...
    # もし5本組が見つからなければ、全線から強制的に5本組を作る（保険）
    if not staves and len(staff_y_coords) >= 5:
        # 単純に最初の5本を使う（本来はより高度な処理が必要だが簡易対応）
        staves.append(staff_y_coords[:5])
    
    if not staves:
        return deskewed_pil

    # 【修正点】音部記号の割り当てを段の高さに基づいて行う
    img_h = deskewed_pil.size[1]
    clefs = []
    for staff in staves:
        staff_center = np.mean(staff)
        # 画像の上半分ならト音記号、下半分ならヘ音記号と仮定
        if staff_center < img_h / 2:
            clefs.append("treble")
        else:
            clefs.append("bass")
